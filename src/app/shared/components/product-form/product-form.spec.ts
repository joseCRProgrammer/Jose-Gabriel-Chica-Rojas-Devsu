...
describe('ProductForm (fechas como string)', () => {
  ...
  describe('ngOnInit()', () => {
    ...
    it('en edit sin initial: solo deshabilita id y no peta', () => {
      component.mode = 'edit';
  ...
  describe('validadores y utilidades', () => {
    ...
    it('urlValidator: ignora espacios al inicio/fin', () => {
      const ctrl = new FormControl('   https://site.com/a.png   ');
      const r = (component as any).urlValidator(ctrl as AbstractControl);
      expect(r).toBeNull();
    });
    ...
    it('releaseTodayOrFuture: valor no fecha → retorna null (rama no válida)', () => {
      const ctrl = new FormControl('no-fecha');
      const r = (component as any).releaseTodayOrFuture(ctrl as AbstractControl);
      expect(r).toBeNull();
    });
    ...
    it('exactPlusOneYearValidator: control vacío → null (rama)', () => {
      const releaseIso = '2024-06-10';
      const validator = (component as any).exactPlusOneYearValidator(releaseIso) as (c: AbstractControl) => any;

      const empty = new FormControl('');
      expect(validator(empty)).toBeNull();
    });
    ...
    it('toIsoDate: Date, timestamp y string inválido', () => {
      const toIso = (component as any).toIsoDate.bind(component) as (v: any) => string;
      const d = new Date(Date.UTC(2030, 4, 9)); // 2030-05-09
      expect(toIso(d)).toBe('2030-05-08');

      const ts = Date.UTC(2031, 0, 1); // number
      expect(toIso(ts)).toBe('2030-12-31');

      expect(toIso('31/12/2030')).toBe('');
    });
  });
  ...
  describe('onSubmit()', () => {
    ...
    it('en edit: emite respetando id deshabilitado', () => {
      component.mode = 'edit';
      component.initial = {
        id: 'fixed-id',
        name: 'X',
      ...
      // Cambiamos resto de campos para ser válido
      setFormValues({
        name: 'Nombre Editado',
        description: 'Descripción editada válida',
        logo: 'https://example.com/logo2.png',
      });

      // Trigger date_release -> date_revision
      const release = '2026-02-10';
      ...
      expect(payload.name).toBe('Nombre Editado');
      expect(payload.logo).toBe('https://example.com/logo2.png');
      expect(payload.date_release).toBe(release);
      expect(payload.date_revision).toBe((component as any).plusOneYear(release));
    });
  });
  ...
  describe('onReset()', () => {
    ...
    it('en edit sin initial: usa id del control deshabilitado y lo conserva', () => {
      component.mode = 'edit';
      component.initial = null;

      // Antes de ngOnInit, setear un id para ver que lo conserva
      component.form.get('id')!.setValue('from-control');
      component.ngOnInit();
      expect(component.form.get('id')!.disabled).toBe(true);
      ...
      expect(component.form.get('id')!.value).toBe('from-control');
      expect(component.form.get('name')!.value).toBe('');
      expect(component.form.get('date_release')!.value).toBeNull();
      expect(component.form.get('date_revision')!.disabled).toBe(true);
      expect(emitSpy).toHaveBeenCalled();
    ...
    it('cambiar date_release a null deja date_revision en null (rama de suscripción)', () => {
      component.ngOnInit();
      component.form.get('date_release')!.setValue('2023-08-10');
      const rev = component.form.get('date_revision')!;
      expect(rev.value).toBe('2024-08-10');

      component.form.get('date_release')!.setValue(null);
      expect(component.form.get('date_revision')!.value).toBeNull();
    });
  });
